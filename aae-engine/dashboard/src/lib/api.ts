...
// Oracle Fusion Observability Types (Phase 6)
export type FusionGoal = {
  goal_id: string;
  objective: string;
...
export type FusionCandidateRanking = {
  goal_id: string;
  candidate_id: string;
  predicted_score: number;
  source: "oracle_native" | "aae_advised" | "hybrid";
  rank: number;
};
...
export type FusionStats = {
  ...
  candidate_rankings: FusionCandidateRanking[];
  acceptance_stats: {
    accepted: number;
    rejected: number;
    total: number;
...
export const api = {
  ...
  fusionStats: () => fetchJson<FusionStats>("/oracle/fusion-stats"),  
};
