...
class EventLog:
    """Compatibility wrapper for EventLog interface.
    
    This class provides the legacy EventLog API while delegating
    to the unified events namespace. For new code, prefer using
    EventStore directly from aae.events.event_store.
    """
    ...
    def _persist(self, event: EventRecord) -> None:
        if not self._log_dir_created and self._log_path:
